...
pub async fn parse_json_request(
    req: Request<Body>,
) -> Result<serde_json::Value, (Response<Body>, String)> {
        ...
        Ok(body_bytes) => match String::from_utf8(body_bytes.to_vec()) {
            Ok(body_string) => {
                match serde_json::from_slice::<serde_json::Value>(body_string.as_bytes()) {
                    Ok(json) => Ok(json),
                    Err(error) => Err(build_error_response(
...
pub fn build_wrong_argument_response(
    argument: &str,
    expected_type: &str,
) -> (Response<Body>, String) {
    build_error_response(
        StatusCode::BAD_REQUEST,
        &format!("\"{}\" is not a {}", argument, expected_type),
    )
}
